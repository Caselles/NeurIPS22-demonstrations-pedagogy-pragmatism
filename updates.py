...
def behavioral_cloning_update_deepsets(model, language, policy_optim, obs_norm, ag_norm, g_norm,
    ...
    _ = model.forward_pass(obs_norm_tensor, ag_norm_tensor, g_norm_tensor, actions=actions_tensor)
    pi, log_pi = model.pi_tensor, model.log_prob
    
    bc_loss = F.mse_loss(pi, actions_tensor)
...
def sil_update_deepsets(model, language, policy_optim, critic_optim, alpha, obs_norm, ag_norm, g_norm,
                    obs_next_norm, ag_next_norm, actions, rewards, returns, args):
    # Tensorize
    obs_norm_tensor = torch.tensor(obs_norm, dtype=torch.float32)
    obs_next_norm_tensor = torch.tensor(obs_next_norm, dtype=torch.float32)
    if language:
    ...
    actions_tensor = torch.tensor(actions, dtype=torch.float32)
    r_tensor = torch.tensor(rewards, dtype=torch.float32).reshape(rewards.shape[0], 1)
    returns_tensor = torch.tensor(returns, dtype=torch.float32).reshape(returns.shape[0], 1)
    ...
    # log pi >> log_pi
    # Rt = sum rewards a partir de t jusqu'a la fin de la demo
    # beta = 0.01 ou 0.05
    # loss weight = 1 ou 0.1
    # correct loss weight at the end of learning
    # v(s) = [ torch.min(qf1_next_target, qf2_next_target) - alpha * log_pi_next ] avec pas le next mais le current

    qf1, qf2 = model.forward_pass(obs_norm_tensor, ag_norm_tensor, g_norm_tensor, actions=actions_tensor)
    _, log_pi = model.pi_tensor, model.log_prob
    v_s = torch.min(qf1, qf2) - alpha * log_pi
    clipped_v_estimation_error = torch.clamp(returns_tensor-v_s, min=0)

    policy_loss = (-log_pi * clipped_v_estimation_error).mean()
    value_loss = 0.5 * F.mse_loss(clipped_v_estimation_error, torch.zeros(clipped_v_estimation_error.shape))

    beta_sil = 0.05

    total_loss = policy_loss + beta_sil * value_loss
    ...
    return policy_loss.item(), value_loss.item(), total_loss.item(), clipped_v_estimation_error
