...
class FetchManipulateEnv(robot_env.RobotEnv):
    ...
    def reset_goal_illustrative_example(self, goal, stack_pos_init=None, biased_init=False, example=1):
        """
        This function resets the environment and target the goal given as input
        Args:
            goal: The semantic configuration to target
        ...
        if type(stack_pos_init) != type(None):
            pos_stack = stack_pos_init[0]
        else:
        ...
        if example==1 or example == 3:
            ...
            pos_two_cubes = [pos_stack, pos_stack]
        for i, obj_name in enumerate(self.object_names):
            object_qpos = self.sim.data.get_joint_qpos('{}:joint'.format(obj_name))
            assert object_qpos.shape == (7,)
            if i in stack:
    ...
    def reset_from_demos(self, states, biased_init=False):
        """
        This function resets the environment and target the goal given as input
        Args:
            goal: The semantic configuration to target
